...
void CHUDQuickInfo::Paint()
{
	...
	C_WeaponBaseBlaster *pBlaster = dynamic_cast<C_WeaponBaseBlaster*>(pWeapon);
	...
	{
		// Check our ammo for a warning
		float ammo = pBlaster->GetHeatCurrent();
		if ( ammo != m_lastAmmo )
		{
			UpdateEventTime();
			m_lastAmmo	= ammo;
			...
			// Find how far through the current clip we are
			float ammoPerc = 1.0f - ( ammo / pBlaster->GetHeatMax() );
	...
	{
		// Update our ammo
		if ( m_ammoFade > 0.0f && pBlaster->IsCooling() && pBlaster->GetCoolTime() - gpGlobals->curtime > 0.5f )
		{
			DrawWarning( xCenter + m_icon_rb->Width(), yCenter, m_icon_rb, m_ammoFade );
		}
		else
		{
			...
			ammoPerc = ammo / pBlaster->GetHeatMax();
			ammoPerc = clamp( ammoPerc, 0.0f, 1.0f );
